...
function raadsR() {
  ...
  const handleSubmit = async (event) => {
    ...
    // Result Message
    const likelihoodMessage = totalScore >= 65 ? 'You are likely autistic.' : 'You are unlikely to be autistic, but further evaluation may be beneficial if concerns persist.';
    const resultMessage = `Based on your total score of ${totalScore}, ${likelihoodMessage}\nSubscale scores are as follows:\n${detailedResults}\n\nPlease consult a professional for an accurate assessment, especially if your scores closely approach or exceed any thresholds.`;
    ...
    try {
      ...
      await axios.post(
        `${BASEURL}api/save_result`,
        { totalScore, subscaleScores }, // Update to match the expected API structure
        config,
      );
    } catch (error) {
